...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_image_analysis_workflow(self, tools, sample_image_path):
        ...
        with patch.object(tools.api_client, 'analyze_image', return_value=iter(mock_api_response)):
            arguments = {
    ...
    @pytest.mark.asyncio
    async def test_full_image_description_workflow(self, tools, sample_image_path):
        ...
        with patch.object(tools.api_client, 'describe_image', return_value=iter(mock_api_response)):
            arguments = {
                "images": [sample_image_path],
    ...
    @pytest.mark.asyncio
    async def test_full_text_extraction_workflow(self, tools, test_image_with_text):
        """Test complete text extraction workflow."""
        mock_api_response = ["No visible text found in this image."]
        
        with patch.object(tools.api_client, 'extract_text', return_value=iter(mock_api_response)):
            arguments = {
    ...
    @pytest.mark.asyncio
    async def test_full_image_comparison_workflow(self, tools, sample_image_path):
        ...
        try:
            ...
            with patch.object(tools.api_client, 'compare_images', return_value=iter(mock_api_response)):
                arguments = {
    ...
    @pytest.mark.asyncio
    async def test_full_object_identification_workflow(self, tools, sample_image_path):
        ...
        with patch.object(tools.api_client, 'identify_objects', return_value=iter(mock_api_response)):
            arguments = {
                "images": [sample_image_path],
    ...
    @pytest.mark.asyncio
    async def test_multiple_images_workflow(self, tools, sample_image_path):
        ...
        try:
            ...
            for i, color in enumerate(['green', 'yellow', 'purple']):
                with tempfile.NamedTemporaryFile(suffix=f'_{color}.png', delete=False) as f:
            ...
            with patch.object(tools.api_client, 'analyze_image', return_value=iter(mock_api_response)):
                arguments = {
                    "images": test_images,
                    "prompt": "Analyze and compare all these images"
                }
    ...
    @pytest.mark.asyncio
    async def test_large_image_compression_workflow(self, tools, large_image_path):
