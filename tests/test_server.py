...
class TestTermiVisServer:
    ...
    async def test_handle_call_tool_success(self, server):
        ...
        with patch.object(server.tools, 'call_tool', return_value=mock_result) as mock_call_tool:
            # Get the handler function
            ...
            mock_call_tool.assert_called_once_with(
                "analyze_image",
                {"images": ["test.jpg"], "prompt": "Test prompt"}
            )
    ...
    async def test_handle_call_tool_error(self, server):
        ...
        with patch.object(server.tools, 'call_tool', side_effect=Exception("Test error")) as mock_call_tool:
            # Get the handler function
            handlers = server.server._tool_call_handlers
            assert len(handlers) == 1
            handler = handlers[0]
    ...
    async def test_run_server(self, server):
        ...
        with patch('src.image_mcp.server.stdio_server') as mock_stdio, \
             ...
             patch.object(server.server, 'create_initialization_options', return_value={}) as mock_init:
            
            # Setup the async context manager
            mock_stdio.return_value.__aenter__ = AsyncMock(return_value=(mock_read_stream, mock_write_stream))
